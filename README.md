...
#### Как работает привязка прокси

- Каждый аккаунт имеет **один прокси** (или работает без него).
- Один и тот же прокси можно привязать к нескольким аккаунтам.
- После `/set_proxy` клиент пересоздаётся с новым прокси немедленно — перезапуск бота не нужен.

#### Где взять прокси

Для Telegram-аккаунтов подходят SOCKS5 и HTTP прокси. Рекомендации по типам:

| Тип | Риск бана | Цена | Подходит для |
|-----|-----------|------|--------------|
| Мобильные (4G) | Низкий | Высокая | Прод, много аккаунтов |
| Резидентные | Низкий | Средняя | Прод |
| Дата-центр IPv4 | Высокий | Низкая | Тест, 1-2 аккаунта |
| Дата-центр IPv6 | Средний | Очень низкая | Тест |

Проверенные сервисы:
- [proxy6.net](https://proxy6.net) — IPv4/IPv6, SOCKS5, от $1-2/мес за IP
- [proxys.io](https://proxys.io) — SOCKS5, IPv4
- [711proxy.com](https://711proxy.com) — мобильные прокси

Если есть собственный VPS — можно поднять SOCKS5 туннель:
```bash
